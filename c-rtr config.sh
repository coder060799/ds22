...
echo '127.0.0.1 c-rtr' > /etc/hosts
echo 'c-rtr' > /etc/hostname
...
# Перезагружаемся и логинимся под cloudadmin

# Запускаем docker-compose.yml для Duplicati
cd /opt/repository/duplicati
docker compose up -d

# Запускаем docker-compose.yml для Wireguard-Portal
cd /opt/repository/wireguard-portal
sudo chmod +x /opt/repository/wireguard-portal/docker-run.sh
sh docker-run.sh